...
export async function getOraclePrice(
    ...
    const stellar_rpc = new rpc.Server(network.rpc);
    ...
    if (rpc.Api.isSimulationSuccess(result)) {
        ...
        if (xdr_str) {
            ...
            if (Array.isArray(price_result) && price_result.length >= 2) {
                return {
                    price: scValToNative(price_result[0] as xdr.ScVal),
                    timestamp: Number(scValToNative(price_result[1] as xdr.ScVal)),
                };
            }
        }
...
export async function getOracleDecimals(
    ...
    if (rpc.Api.isSimulationSuccess(result) && result.result) {
        ...
        throw new Error(`Failed to fetch oralce decimals${'error' in result && result.error ? `: ${result.error}` : ''}`);
    }
}