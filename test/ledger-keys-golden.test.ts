...
describe('golden ledger-key encoding vs @stellar/stellar-sdk spec encoder', () => {
    it('instance-tier unit variants match the spec encoder byte-for-byte', () => {
        ...
        for (const [tag, built] of cases) {
            expect(b64(built), `unit variant ${tag}`).toBe(
                b64(specScVal(dataKey, tag)),
            );
        }
    });
